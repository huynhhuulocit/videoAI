...
- User có thể mở menu `Scripts` (route tương thích `/shots`) để xem danh sách script/shot plan reusable, thêm/sửa ở trang riêng `/shots/new` hoặc `/shots/{shotPlanId}`, xóa/archive và chọn default. Khi tạo/sửa, user nhập nội dung câu chuyện trong textarea riêng, thêm attribute cấp shot plan, xem raw provider request và raw response JSON dạng thu gọn/mở rộng, chỉnh sửa nhiều shot video tối đa 8 giây/shot, mỗi shot có `Start state`, `End state` và `Dialogue`, rồi chọn shot plan đó trong bất kỳ project workspace nào của cùng user.
...
- Trong project workspace, các action AI dùng master prompt có button `Prompt` trước `Request` để xem prompt đầy đủ sau khi thay placeholder và nối runtime context.
...
- User có thể mở `Kịch bản` để xem danh sách scenario, thêm/sửa ở `/templates/new` hoặc `/templates/{templateId}`, xóa/archive và chọn default. Scenario gồm attribute/option có thể tạo bằng AI hoặc thủ công; trang thêm/sửa hiển thị `Scenario` master prompt có thể sửa tạm cho lần tạo AI, hiển thị lỗi provider chi tiết nếu thất bại và không fallback sang dữ liệu mẫu.
...
- Trong `Script Flow`, `Story Content` là Bước 1, `Kịch bản tạo prompt` là Bước 2, `Shots tạo prompt` là Bước 3, các step có thể thu gọn/mở rộng. Story Content có `Scripts` master prompt có thể sửa tạm thời; response tạo bằng AI được ghi lại vào textarea Story Content và dùng chung cho phân tích kịch bản, tạo shots, compose prompt từng shot và tạo script.
...
- Admin có thể quản lý `Master Prompt` tại route tương thích `/admin/shot-prompt`, gồm 3 nhóm `Scenario`, `Shots`, `Scripts`, mỗi nhóm có create/edit/delete và một default active.
- Master prompt giữ format placeholder khuyến nghị theo từng nhóm, nhưng không bắt buộc placeholder khi lưu; backend replace nếu có và nối runtime context có cấu trúc sau prompt được chọn.
