...
- Trong `Scenario`, Bước 3 hiển thị `Shots` master prompt do admin quản lý trong textarea có thể sửa tạm thời; prompt override này chỉ dùng cho lần tạo shots hiện tại.
...
- Admin manages Story, Scenario, and Shots as separate parent groups. Each group has a Master Prompt page and an Attribute page.
...
- Master Prompt Config is also separate from workflow Attribute catalogs. It is admin-only, shared globally across Story/Scenario/Shots master prompt editors, and user workflows cannot select or mutate it.
- Project and One Click workflows load active admin catalogs for Story, Scenario, and Shots. Required attributes auto-select the first option and cannot be empty, while users can still multi-select and change options.
