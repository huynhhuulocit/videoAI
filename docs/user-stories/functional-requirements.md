...
- `Bước 1` phải hiển thị textarea `Scripts`/Story Content master prompt do admin quản lý và có thể sửa tạm thời. Khi user bấm tạo bằng AI, response trả về được ghi lại vào textarea `Story Content`.
...
- Các action AI dùng master prompt có placeholder trong project workspace phải có button `Prompt` nằm trước `Request`. Button này mở popup read-only hiển thị prompt đầy đủ sau khi thay placeholder optional và nối runtime context, để user kiểm tra trước khi gọi AI.
...
- Trong panel `Attributes` của workspace, attribute/option có translate/mô tả từ Scenario phải có icon helper; hover hoặc click icon sẽ hiển thị translate và mô tả mà không làm đổi trạng thái chọn/collapse.
...
- Request `Phân tích kịch bản` phải kết hợp master prompt, nội dung kịch bản user nhập và catalog attribute/option của Kịch bản đang chọn. Placeholder `{story}` và `{attributes}` được hỗ trợ nhưng không bắt buộc vì backend nối runtime context có cấu trúc.
...
- Admin có thể quản lý 3 nhóm master prompt: `Scenario`, `Shots`, `Scripts`.
...
- Master prompt giữ format placeholder khuyến nghị theo từng nhóm, nhưng không bắt buộc placeholder khi lưu; backend replace nếu có và luôn nối runtime context có cấu trúc sau prompt được chọn.
- `Scenario` dùng làm default khi user bấm `Phân tích kịch bản`; `Shots` dùng cho workflow AI tạo shot plan và `/shots`; `Scripts` dùng cho Script Flow prompt/content generation.
...
- Route `/templates` chỉ hiển thị danh sách kịch bản, gồm action thêm, sửa, xóa/archive và `Set default`.
...
- Request AI tạo kịch bản phải gửi optional temporary `masterPrompt`, thay placeholder `{story}`/`{attributes}` nếu có, và backend phải nối runtime context có cấu trúc trước khi gọi provider.
...
- Schema ngắn và JSON có thể kèm tiếng Việt/mô tả để người dùng đọc hiểu attribute/option. Tiếng Việt/mô tả chỉ lưu vào `description`, không thay thế label/value tiếng Anh dùng cho xử lý prompt.
- Trang thêm/sửa scenario có textarea `Vietnamese translate JSON` để tạo, chỉnh và apply bản dịch/giải thích tiếng Việt cho attribute/option hiện tại bằng `attributeId`/`optionId`; khi JSON hiện tại đã được apply và không còn thay đổi description, nút apply bị disable.
- User có thể tự thêm attribute.
- User có thể tự thêm option cho từng attribute.
- User có thể xóa kịch bản đã lưu.
- User có thể chọn một kịch bản default; kịch bản default được ưu tiên hiển thị đầu danh sách và trong selector ở project workspace.
...
- Route `/shots` chỉ hiển thị danh sách script/shot plan, gồm action thêm, sửa, xóa/archive và `Set default`.
